...
<p>
  ...
  <a href='https://github.com/HubSpot/cms-theme-boilerplate'>&nbsp;here</a>. Make
  sure you generally know what's going on, then run this command in Terminal—inside
...
<p>
  ...
  but at the moment it needs some tweaks to support Webpack 5 + the related
  plugin updates
</p>
...
<p>
  Run the start script to kickoff webpack and autoupload. The script runs
  webpack, watches for changes, sets the mode to development, and passes in a
  "autoupload" environment variable that defaults to true. The webpack config is
  also expecting a "account" environment variable but that is automatically
  passed in via your hubspot.config.yml file so it's not needed (but you can add
  it if you want).
</p>

<p>
  As you save files in `src`, your `dist/staging` site will automatically update
...
<h2>Related ideas</h2>
<p>
  A way to provide feedback about specific parts of this "guide" would be really
  helpful. I wonder if I can build a confusing indicator of some sort? Any
  ideas?
</p>
